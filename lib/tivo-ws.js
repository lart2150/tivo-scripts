...
import {URL} from 'url';
import fs from 'fs';
import { getDomainToken } from './getDomainToken.js';
...
export class TivoWs {
     ...
     */
    constructor(username, password, tsn) {
        this.rpcId = 0;
        this.received = [];
    ...
    async connect () {
        if (!this.domainToken) {
            const token = await getDomainToken(this.username, this.password);
            this.domainToken = token.token;
        }
        this.disconnect();
        this.received = [];
